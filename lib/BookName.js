 ...
 * @constructor
 */
function BookName(taxpayerIdentityNumber, accountingPeriod, bookTypeIdentifier, bookContainsInformation, currencyCode) {
  let _args = {
...
function _getFinancialStatementsCode() {
  ...
  if (withoutFinancialStatementsCode === true) {
    ...
    throw new Error(`El código de libro electrónico ingresado, no está soportado (${this.bookTypeIdentifier})`)
  }
}
...
function _getCurrencyUsedIdentifier() {
  ...
  switch (this.currencyCode) {
    case CURRENCY_CODES.PEN:
      ...
      break;
    default:
      throw new Error(`El código internacional de moneda ingresado, no está permitido (${this.currencyCode})`)
  }
...
function _getAccountingPeriodInformation(format) {
  ...
  if (validAccountingPeriod === true) {
    ...
    throw new Error('El período contable es incorrecto')
  }
}
...
function _getBookContentIdentifier() {
  if (this.bookContainsInformation != null) {
    ...
    throw new Error(`El indicador de contenido del libro, no ha sido definido correctamente (${this.bookContainsInformation})`)
  }
}
