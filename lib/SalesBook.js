...
SalesBook.prototype.addSale = function(payload) {
  ...
  try {
    ...
    if (result.error) {
      throw new Error('Error en la validaciones')
    }
...
function _getValidationSchema() {
  let documentSchema = Joi.object().keys({
    ...
    issuanceDate: Joi.string().required(), //Joi.date().format('YYYY-MM-DD').required(),
    expirationDate: Joi.string().default(null) //Joi.date().format('YYYY-MM-DD').default(null)
  })
  ...
  const currencySchema = Joi.object().keys({
    ...
    exchangeRate: Joi.alternatives().when('code', {
      is: CURRENCY_CODES.PEN,
      then: Joi.number().valid(1).required(),
      otherwise: Joi.alternatives().when('code', {
