...
const IDENTITY_DOCUMENT_TYPES = require('./constants/IdentityDocumentTypes')
...
function _getValidationSchema() {
  ...
  const customerSchema = Joi.object().keys({
    identityType: Joi.string().valid([
      IDENTITY_DOCUMENT_TYPES.DNI,
      IDENTITY_DOCUMENT_TYPES.RUC
    ]).required(),
    identityNumber: Joi.alternatives()
      .when('identityType', {
        is: IDENTITY_DOCUMENT_TYPES.DNI,
        then: Joi.string().regex(/^[0-9]{8}$/).required()
      })
      .when('identityType', {
        is: IDENTITY_DOCUMENT_TYPES.RUC,
        then: Joi.string().regex(/^[0-9]{11}$/).required()
      }),
