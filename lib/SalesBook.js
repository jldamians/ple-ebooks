...
SalesBook.prototype.addSale = function(payload) {
  ...
  const accountingPeriod = moment(payload.accountingPeriod, 'YYYYMM')

  // correlativo del mes
  sale.sequential = sequential

  // periodo contable actual
  sale.accountingPeriod = accountingPeriod

  // regimen laboral del contribuyente
  sale.taxpayerRegimenCode = payload.taxpayerRegimenCode
  ...
  sale.cpe = new CPE(
    ...
    payload.cpe.serial,
    payload.cpe.number,
    payload.cpe.issuanceDate,
    payload.cpe.expirationDate != null ? payload.cpe.expirationDate : null
  )
  ...
  if (payload.hasOwnProperty('taxableAmount')) {
    sale.payableAmount = payload.taxableAmount
  }

  // importe exportación
  if (payload.hasOwnProperty('exportAmount')) {
    sale.payableAmount = payload.exportAmount
  }

  // importe exonerado
  if (payload.hasOwnProperty('exemptAmount')) {
    sale.payableAmount = payload.exemptAmount
  }

  // importe inafecto
  if (payload.hasOwnProperty('nonTaxableAmount')) {
    sale.payableAmount = payload.nonTaxableAmount
  }

  // importe otros cargos
  if (payload.hasOwnProperty('otherChargesAmount')) {
    sale.payableAmount = payload.otherChargesAmount
  }
