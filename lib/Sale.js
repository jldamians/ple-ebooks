...
function _buildSale() {
  ...
  const exemptAmount = utils.fillDecimals(this.exemptAmount, 2)
  const nonTaxableAmount = utils.fillDecimals(this.nonTaxableAmount, 2)
  ...
  const payableAmount = utils.fillDecimals(this.payableAmount, 2)
  const currencyCode = this.currency.code
  const currencyExchangeRate = utils.fillDecimals(this.currency.exchangeRate, 3)
...
function _getCuo() {
  ...
  if (this.regimeType === TAXPAYER_REGIMEN_CODES.special) {
    ...
    throw new Error(`El régimen ${this.regimeType} no está soportado`)
  }
}
...
function _getAccountingEntryNumber() {
  ...
  if (this.regimeType === TAXPAYER_REGIMEN_CODES.special) {
    ...
    throw new Error(`El regimen ${this.regimeType} no está soportado`)
  }
}
...
function _getTaxableAmountDiscount() {
  ...
  if (taxableOperationWithDiscount) {
    return utils.fillDecimals(
      (Math.abs(this.igv.taxableAmount) * -1), 2
    )
  } else {
    return ''
  }
...
function _getTaxAmountDiscount() {
  ...
  if (taxableOperationWithDiscount) {
    return utils.fillDecimals(
      (Math.abs(this.igv.taxAmount) * -1), 2
    )
  } else {
    return ''
  }
...
function _getTaxableAmount() {
  ...
  if (taxableOperationWithoutDiscount) {
    return utils.fillDecimals(
      Math.abs(this.igv.taxableAmount), 2
    )
  } else {
    return ''
  }
