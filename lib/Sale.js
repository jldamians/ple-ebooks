...
function Sale() {
  ...
  Object.defineProperty(this, 'taxpayerRegimenCode', {
    ...
    set: (value) => { _args.taxpayerRegimenCode = value }
  })
...
function _getAnnotationOpportunityStatus() {
  ...
  if (issuedInsideAccountingPeriod) {
    ...
    throw new Error(
      `La fecha de emisión del cpe (${cpeIssuanceDate.format('MM/YYYY')}) debe corresponder al período actual (${cpeAccountingPeriod.format('MM/YYYY')})`
    )
  }
}
